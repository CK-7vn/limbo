...
def test_output_file():
    ...
    output_file = shell.config.test_dir / output_filename

    shell.execute_dot(".open testing/testing.db")
    shell.execute_dot(f".cd {shell.config.test_dir}")
    ...
    expected_lines = {
        ...
        f"CWD: {os.getcwd()}/{shell.config.test_dir}": "Working directory changed",
...
def test_import_csv_skip():
    ...
    shell.run_test("verify-csv-skip", "select * from csv_table;", "3\n|4.0\n|String2")
    shell.quit()
